...
# checks the user wrapper soft connections and CVC. There's no source, so LVS isn't possible.

# usage: check_user_wrapper.sh [<slot_list>]
...
for slot_index in "${slot_list[@]}"; do
	...
	if [[ -f user_top ]]; then
		...
		UPRJ_ROOT=$PWD WORK_ROOT=$PWD/work/$user_top $LVS_ROOT/run_be_checks lvs/$user_top/lvs_config.json $user_top $layout_top *gds.gz 2>&1 |
			tee check.out
	else
